...
def rotate_and_remove(queue: deque[int], k: int) -> int:
    ...
    """
    queue.rotate(-(k - 1))  # 왼쪽으로 k-1칸 회전
    return queue.popleft()  # 앞에서 제거 후 반환
...
def josephus_problem(n: int, k: int) -> list[int]:
    ...
    """
    queue = create_circular_queue(n)  # 1. 원형 큐 생성
    result = []

    while queue:
        removed = rotate_and_remove(queue, k)  # 2. k번째 사람 제거
        result.append(removed)                 # 3. 제거 순서 기록
