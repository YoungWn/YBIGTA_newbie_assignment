...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h1 = X @ W1 + b1  # (N, D) @ (D, H1) + (1, H1) -> (N, H1)
        z1 = relu(h1)  # ReLU activation
        
        h2 = z1 @ W2 + b2  # (N, H1) @ (H1, H2) + (1, H2) -> (N, H2)
        z2 = relu(h2)  # ReLU activation

        h3 = z2 @ W3 + b3  # (N, H2) @ (H2, H3) + (1, H3) -> (N, H3)
        z3 = relu(h3)  # ReLU activation

        h4 = z3 @ W4 + b4  # (N, H3) @ (H3, C) + (1, C) -> (N, C)
        y_hat = sigmoid(h4)  # Sigmoid activation for binary classification
...
def relu(x):
    ### CODE HERE ###
    # ReLU = max(0, x)
    # 출처: https://docs.pytorch.org/docs/stable/generated/torch.nn.ReLU.html
    x = np.maximum(0, x)
    ############################
    return x
...
def leakyrelu(x):
    ...
    # 출처: https://docs.pytorch.org/docs/stable/generated/torch.nn.LeakyReLU.html
